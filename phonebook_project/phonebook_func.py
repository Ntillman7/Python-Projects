...
def ask_quit(self):
    if messagebox.askokcancel("Exit program", "Okay to exit application?"):
        #messagebox is tkinter class with methods, exit program title of box
        # This closes app
        self.master.destroy()
        os._exit(0)
        #takes widgets and deletes it from memory, clears out users memory

#=========================================================
def create_db(self):
    conn = sqlite3.connect('phonebook.db')
    #creating class object called connection
    with conn:
        cur = conn.cursor()
...
def count_records(cur):
    ...
    #accesses the database and counts all the rows, extracts the data
    return cur,count
...
def onSelect(self,event):
    ...
    #[0] catches the index they are clicking on
    value = varList.get(select)
    #gets the text at the index number
    conn = sqlite3.connect('phonebook.db')
    with conn:
        cursor = conn.cursor()
...
def addToList(self):
    ...
    var_fname = var_fname.strip() # Strip will remove any blank spaces before and after the user's entry
    var_lname = var_lname.strip() 
    var_fname = var_fname.title() # Title will ensure that the first character in each word is capitalized
    var_lname = var_lname.title()
    var_fullname = ("{} {}".format(var_fname,var_lname)) # combine our normailzed names into a fullname
    print("var_fullname: {}".format(var_fullname)) #silent, only for development
    var_phone = self.txt_phone.get().strip()
    var_email = self.txt_email.get().strip()
    if not "@" or not "." in var_email: # will use this soon
        print("Incorrect email format!!!")
    if (len(var_fname) > 0) and (len(var_lname) > 0) and (len(var_phone) > 0) and(len(var_email) > 0): # enforce the user to provide both names, checks to see if they are empty
        conn = sqlite3.connect('phonebook.db')
        with conn:
            cursor = conn.cursor()
            # Check the database for existance of the fullname, if so we will alert user and disregard request
            cursor.execute("""SELECT COUNT(col_fullname) FROM tbl_phonebook WHERE col_fullname = '{}'""".format(var_fullname))#,(var_fullname)) seeing if fullname exists
